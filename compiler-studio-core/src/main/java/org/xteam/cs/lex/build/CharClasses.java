...
public class CharClasses {
    ...
    /**
     * Constructs a new CharClass object that provides space for classes of
     * characters from 0 to maxCharCode.
     * 
     * Initially all characters are in class 0.
     * 
     * @param maxCharCode
     *            the last character code to be considered. (127 for 7bit
     *            Lexers, 255 for 8bit Lexers and 0xFFFF for Unicode Lexers).
     */
    public CharClasses(int maxCharCode) {
        ...
        classes.add(new IntCharSet(new IntCharSet.Interval((char) 0, maxChar)));
    }
     ...
     * 
     * @param charCode
     *            the largest character code, used for the scanner (i.e. %7bit,
     *            %8bit, %16bit etc.)
     */
    public void setMaxCharCode(int charCode) {
        if (charCode < 0 || charCode > maxChar)
            throw new IllegalArgumentException();
     ...
     * equivalence class with characters that are not elements of
     * <code>set</code>.
     * 
     * @param set
    ...
    /**
     * Returns the code of the character class the specified character belongs
     * to.
     */
    public int getClassCode(char letter) {
        int i = -1;
        while (true) {
            IntCharSet x = (IntCharSet) classes.get(++i);
    ...
    public void makeClass(String str, boolean caseless) {
        for (int i = 0; i < str.length(); i++)
            makeClass(str.charAt(i), caseless);
    }
     ...
     * gets a new character class.
     * 
     * Characters that are elements of the set <code>v</code> are not in the
     * same equivalence class with characters that are not elements of the set
     * <code>v</code>.
     * 
     * @param v
     *            a Vector of Interval objects. This Vector represents a set of
     *            characters. The set of characters is the union of all
     *            intervalls in the Vector.
     * 
     * @param caseless
     *            if true upper/lower/title case are considered equivalent
     */
    public void makeClass(List<IntCharSet.Interval> v, boolean caseless) {
     ...
     *            if true upper/lower/title case are considered equivalent
     */
    public void makeClassNot(List<IntCharSet.Interval> v, boolean caseless) {
        makeClass(new IntCharSet(v), caseless);
    }

    /**
     * Returns an array that contains the character class codes of all
     * characters in the specified set of input characters.
     */
     ...
     * 
     * @param intervallVec
     *            a Vector of Intervalls, the set of characters to get the class
     *            codes for
     * 
     * @return an array with the class codes for intervallVec
     */
    public int[] getClassCodes(List<IntCharSet.Interval> intervallVec) {
        return getClassCodes(new IntCharSet(intervallVec), false);
    }

    /**
     * Returns an array that contains the character class codes of all
     * characters that are <strong>not</strong> in the specified set of input
     * characters.
     * 
     * @param intervallVec
     *            a Vector of Intervalls, the complement of the set of
     *            characters to get the class codes for
     * 
     * @return an array with the class codes for the complement of intervallVec
     */
    public int[] getNotClassCodes(List<IntCharSet.Interval> intervallVec) {
     ...
     * 
     * all classes must be disjoint, checks if all characters have a class
     * assigned.
     */
    public void check() {
        for (int i = 0; i < classes.size(); i++)
            for (int j = i + 1; j < classes.size(); j++) {
                IntCharSet x = (IntCharSet) classes.get(i);
                IntCharSet y = (IntCharSet) classes.get(j);
                if (x.and(y).containsElements()) {
                    System.out.println("Error: non disjoint char classes " + i
                            + " and " + j);
    ...
    public CharClassInterval[] getIntervals() {
        ...
        for (i = 0; i < size; i++)
            numIntervalls += ((IntCharSet) classes.get(i))
                    .numIntervalls();
        ...
        while (i < numIntervalls) {
            ...
            IntCharSet set = (IntCharSet) classes.get(code);
            IntCharSet.Interval iv = set.getNext();
