...
public class TestConcreteParserEditor extends Editor {

	private static final long serialVersionUID = 5652894461959722141L;
	
	private AnnotableEditorPane editor;
	private JTree tree;
	private ParseTreeModel treeModel;

	public TestConcreteParserEditor(TestConcreteParserResource resource, StyleManager styleManager) {
		super(resource);
	...
	private static class ParseTreeModel implements TreeModel {
		...
		public ParseTreeModel(TestConcreteParserResource resource) {
		...
		public int getChildCount(Object parent) {
			if (parent instanceof RuleNode) {
				RuleNode rn = (RuleNode) parent;
				return rn.getChildren().size();
			}
			return 0;
		}
